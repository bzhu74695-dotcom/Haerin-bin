...
const getImageFullSrc = image => image.fullSrc || image.fullsrc || image.largeSrc || image.originalSrc || image.src || '';
...
function buildItems(pool, seg) {
  ...
  const normalizedImages = pool.map(image => {
    ...
    return { src: image.src || '', fullSrc: getImageFullSrc(image), alt: image.alt || '' };
  });
...
export default function DomeGallery({
  ...
  useEffect(() => {
    ...
    const tick = time => {
      ...
      if (!draggingRef.current && !focusedElRef.current && !openingRef.current && !inertiaRAF.current) {
        const nextX = clamp(rotationRef.current.x, -maxVerticalRotationDeg, maxVerticalRotationDeg);
  ...
  const closeFocusedItem = useCallback(() => {
    ...
    if (originalImg) {
      ...
      img.style.cssText = 'width:100%;height:100%;object-fit:contain;background:rgba(5,4,6,.78);';
    ...
    >
      <div ref={mainRef} className="sphere-main">
        ...
        <div className="viewer" ref={viewerRef}>
          ...
          <button className="dg-close-button noselect" type="button" aria-label="Close full image" onClick={closeFocusedItem}>
